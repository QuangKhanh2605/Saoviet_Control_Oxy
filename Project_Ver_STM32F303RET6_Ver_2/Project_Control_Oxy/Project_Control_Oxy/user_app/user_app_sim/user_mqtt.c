...
uint8_t _mDATA_HANDSHAKE(int Kind_Send)
{
    ...
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0x02;
    ...
    {
        *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = (sFreqInfor.FreqSendOnline_u32 >> 8) & 0xFF; //default FreqSendUnitMin_u32
        *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sFreqInfor.FreqSendOnline_u32 & 0xFF;     //default FreqSendUnitMin_u32
    } else
    {
	...
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = OBIS_ENVI_SET_OXY_UPPER; // obit
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0x02; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sParamCtrlOxy.Oxy_Upper>>8; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sParamCtrlOxy.Oxy_Upper; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0xFE; // length
    
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = OBIS_ENVI_SET_OXY_LOWER; // obit
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0x02; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sParamCtrlOxy.Oxy_Lower>>8; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sParamCtrlOxy.Oxy_Lower; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0xFE; // length
    
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = OBIS_ENVI_STATE_MACHINE; // obit
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0x01; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sStateCtrlOxy.StateMachine; // length
    
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = OBIS_ENVI_SET_PH_UPPER; // obit
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0x02; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sParamCtrlPH.pH_Upper>>8; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sParamCtrlPH.pH_Upper; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0xFE; // length
    
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = OBIS_ENVI_SET_PH_LOWER; // obit
	*(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0x02; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sParamCtrlPH.pH_Lower>>8; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = sParamCtrlPH.pH_Lower; // length
    *(sMQTT.sPayload.Data_a8 + sMQTT.sPayload.Length_u16++) = 0xFE; // length
#endif
...
//    uint32_t  TempU32 = 0;
    ...
    uint32_t Temp_OxyUpper = 0;
    uint32_t Temp_OxyLower = 0;
    uint8_t  Flag_Recv_OxyUpper = 0;
    uint8_t  Flag_Recv_OxyLower = 0;
    
    uint32_t Temp_pH_Upper = 0;
    uint32_t Temp_pH_Lower = 0;
    uint8_t Flag_Recv_pH_Upper = 0;
    uint8_t Flag_Recv_pH_Lower = 0;
    
    uint8_t aData_Res[50] = {0};
    uint8_t Length_Res = 0;
    ...
    while ((PosFix + 4) <= str_Receiv->Length_u16)   //vi byte cuoi la crc
    {
        ...
        switch(ObisConfig)   //luon luon co 1 byte crc phía sau
        {
            ...
            case OBIS_FREQ_SEND:
                TempU16 = 0;
                TempLeng = *(str_Receiv->Data_a8 + PosFix++);
                aData_Res[Length_Res++] = OBIS_FREQ_SEND;
                aData_Res[Length_Res++] = TempLeng;
                for (i = 0; i < TempLeng; i++)
                    TempU16 = (TempU16 << 8 ) | *(str_Receiv->Data_a8 + PosFix++);

                TempU16 = Calculator_Value_Scale(TempU16, *(str_Receiv->Data_a8 + PosFix++), 0x00);
                sFreqInfor.FreqSendOnline_u32 = TempU16;
                aData_Res[Length_Res++] = sFreqInfor.FreqSendOnline_u32 >> 8;
                aData_Res[Length_Res++] = sFreqInfor.FreqSendOnline_u32;
                aData_Res[Length_Res++] = 0x00;
                //Luu lai
                Save_Freq_Send_Data ();
                //Set lai timer
                AppComm_Set_Next_TxTimer();
//                DCU_Respond(_AT_REQUEST_SERVER, (uint8_t *)"OK", 2, 0);
                break;
                
            case OBIS_ENVI_SET_OXY_UPPER:
                Temp_OxyUpper = 0;
                Flag_Recv_OxyUpper = 1;
                TempLeng = *(str_Receiv->Data_a8 + PosFix++);
                for (i = 0; i < TempLeng; i++)
                    Temp_OxyUpper = (Temp_OxyUpper << 8 ) | *(str_Receiv->Data_a8 + PosFix++);
                
                Temp_OxyUpper = Calculator_Value_Scale(Temp_OxyUpper, *(str_Receiv->Data_a8 + PosFix++), 0xFE);
                break;
                
            case OBIS_ENVI_SET_OXY_LOWER:
                Temp_OxyLower = 0;
                Flag_Recv_OxyLower = 1;
                TempLeng = *(str_Receiv->Data_a8 + PosFix++);
                for (i = 0; i < TempLeng; i++)
                    Temp_OxyLower = (Temp_OxyLower << 8 ) | *(str_Receiv->Data_a8 + PosFix++);
                
                Temp_OxyLower = Calculator_Value_Scale(Temp_OxyLower, *(str_Receiv->Data_a8 + PosFix++), 0xFE);
                break;
                
            case OBIS_ENVI_SET_PH_UPPER:
                Temp_pH_Upper = 0;
                Flag_Recv_pH_Upper = 1;
                TempLeng = *(str_Receiv->Data_a8 + PosFix++);
                for (i = 0; i < TempLeng; i++)
                    Temp_pH_Upper = (Temp_pH_Upper << 8 ) | *(str_Receiv->Data_a8 + PosFix++);
                
                Temp_pH_Upper = Calculator_Value_Scale(Temp_pH_Upper, *(str_Receiv->Data_a8 + PosFix++), 0xFE);
                break;
                
            case OBIS_ENVI_SET_PH_LOWER:
                Temp_pH_Lower = 0;
                Flag_Recv_pH_Lower = 1;
                TempLeng = *(str_Receiv->Data_a8 + PosFix++);
                for (i = 0; i < TempLeng; i++)
                    Temp_pH_Lower = (Temp_pH_Lower << 8 ) | *(str_Receiv->Data_a8 + PosFix++);
                
                Temp_pH_Lower = Calculator_Value_Scale(Temp_pH_Lower, *(str_Receiv->Data_a8 + PosFix++), 0xFE);
                break;
...
//              break;
    ...
    if(Flag_Recv_OxyUpper == 1 || Flag_Recv_OxyLower == 1)
    {
        if(Flag_Recv_OxyUpper == 0)
          Temp_OxyUpper = sParamCtrlOxy.Oxy_Upper;
        
        if(Flag_Recv_OxyLower == 0)
          Temp_OxyLower = sParamCtrlOxy.Oxy_Lower; 
        
        if(Temp_OxyUpper >= (Temp_OxyLower + 10) && Temp_OxyUpper <= OXY_UPPER_MAX)
        {
            if(Temp_OxyLower <= Temp_OxyUpper -10 && Temp_OxyLower >= OXY_LOWER_MIN)
            {
                Save_OxyUpperLower(Temp_OxyUpper, Temp_OxyLower, sParamCtrlOxy.Oxy_Upper_Warning, sParamCtrlOxy.Oxy_Lower_Warning);
//                DCU_Respond (_AT_REQUEST_SERVER, (uint8_t *)"OK", 2, 1);
            }
//            else
//                DCU_Respond(_AT_REQUEST_SERVER, (uint8_t *)"FAIL", 4, 0);
...
//            DCU_Respond(_AT_REQUEST_SERVER, (uint8_t *)"FAIL", 4, 0);
    ...
    if(Flag_Recv_OxyUpper == 1)
    {
        aData_Res[Length_Res++] = OBIS_ENVI_SET_OXY_UPPER;
        aData_Res[Length_Res++] = 0x02;
        aData_Res[Length_Res++] = sParamCtrlOxy.Oxy_Upper >>8;
        aData_Res[Length_Res++] = sParamCtrlOxy.Oxy_Upper;
        aData_Res[Length_Res++] = 0xFE;
    }
    if(Flag_Recv_OxyLower == 1)
    {
        aData_Res[Length_Res++] = OBIS_ENVI_SET_OXY_LOWER;
        aData_Res[Length_Res++] = 0x02;
        aData_Res[Length_Res++] = sParamCtrlOxy.Oxy_Lower >>8;
        aData_Res[Length_Res++] = sParamCtrlOxy.Oxy_Lower;
        aData_Res[Length_Res++] = 0xFE;
    }

    if(Flag_Recv_pH_Upper == 1 || Flag_Recv_pH_Lower == 1)
    {
        if(Flag_Recv_pH_Upper == 0)
          Temp_pH_Upper = sParamCtrlPH.pH_Upper;
        
        if(Flag_Recv_pH_Lower == 0)
          Temp_pH_Lower = sParamCtrlPH.pH_Lower; 
        
        if(Temp_pH_Upper >= (Temp_pH_Lower + 10) && Temp_pH_Upper <= PH_UPPER_MAX)
        {
            if(Temp_pH_Lower <= Temp_pH_Upper -10 && Temp_pH_Lower >= PH_LOWER_MIN)
            {
                Save_pH_UpperLowerWar(Temp_pH_Upper, Temp_pH_Lower, sParamCtrlPH.pH_StepWarning);
            }
        }
    }
    if(Flag_Recv_pH_Upper == 1)
    {
        aData_Res[Length_Res++] = OBIS_ENVI_SET_PH_UPPER;
        aData_Res[Length_Res++] = 0x02;
        aData_Res[Length_Res++] = sParamCtrlPH.pH_Upper >>8;
        aData_Res[Length_Res++] = sParamCtrlPH.pH_Upper;
        aData_Res[Length_Res++] = 0xFE;
    }
    if(Flag_Recv_pH_Lower == 1)
    {
        aData_Res[Length_Res++] = OBIS_ENVI_SET_PH_LOWER;
        aData_Res[Length_Res++] = 0x02;
        aData_Res[Length_Res++] = sParamCtrlPH.pH_Lower >>8;
        aData_Res[Length_Res++] = sParamCtrlPH.pH_Lower;
        aData_Res[Length_Res++] = 0xFE;
    }

    if(Length_Res > 0)
        DCU_Respond(_AT_REQUEST_SERVER, aData_Res, Length_Res, 0);
...
void mConnect_MQTT (void)
{
    ...
    {
        Connect_Packet.username.cstring = (char*)sSimInfor.sServer.sUserNameMQTTBackUp.Data_a8;
        Connect_Packet.password.cstring = (char*)sSimInfor.sServer.sPasswordMQTTBackUp.Data_a8;
    }
