...
static uint8_t _Cb_Uart_Rx_Debug(uint8_t event)
{
    ...
    uint8_t aDataCheck_Debug[12] = "at+userdebug";
    sData CheckRecv_Debug;
    CheckRecv_Debug.Data_a8 = aDataCheck_Debug;
    CheckRecv_Debug.Length_u16 = 12;
    
    uint8_t aDataCheck_Slave[12] = "at+userslave";
    sData CheckRecv_Slave;
    CheckRecv_Slave.Data_a8 = aDataCheck_Slave;
    CheckRecv_Slave.Length_u16 = 12;
    ...
    } else
    {
        if (sUartDebug.Length_u16 == LastLengthRecv)
        {
            ...
            Pos_Str_Debug = Find_String_V2(&sUartDebug, &CheckRecv_Debug);
            Pos_Str_Slave = Find_String_V2(&sUartDebug, &CheckRecv_Slave);
            if(Pos_Str_Slave >= 0)
            {
                Flag_User_Rs485Slave = _RS485_USER_SLAVE;
            }
            else if(Pos_Str_Debug >= 0)
            {
                Flag_User_Rs485Slave = _RS485_USER_DEBUG;
                fevent_enable(sEventAppRs485Slave, _EVENT_RS485_SEND_SLAVE);
            }
            
            if(Flag_User_Rs485Slave == _RS485_USER_SLAVE)
            {
                fevent_active(sEventAppRs485Slave, _EVENT_RS485_SLAVE_HANDLE_COMPLETE);
            }
            else
            {
...
void DeInit_PeriodTimer(void)
{
#ifdef USING_APP_OXYGEN
    AppCtrlOxy_Log_Data_TSVH();    
#endif
    UTIL_TIMER_Stop(&TimerTx);
	...
	} else
    {
        if (sModem.ModeSimPower_u8 == _POWER_MODE_ONLINE)
        {
            ...
            {
                FreqCacul = FreqCacul * 60 - sRTC.sec + 2 + sFreqInfor.FreqSendUnitMin_u32*60;
                FreqCacul *= 1000;
            }
        } else
        {
