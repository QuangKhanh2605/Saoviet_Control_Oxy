...
static uint8_t fevent_ctrl_oxy_read_power(uint8_t event);
...
//  {_EVENT_CTRL_OXY_SW_NOTIFY,       1, 5, 500,          fevent_ctrl_oxy_sw_notify},
  ...
  {_EVENT_CTRL_OXY_READ_POWER,      1, 0, 0,            fevent_ctrl_oxy_read_power},
  ...
  {_EVENT_CTRL_OXY_LED_WARNING,     0, 5, 1000,         fevent_ctrl_oxy_led_warning},
...
//  {_EVENT_CTRL_OXY_FREQ_NOTIFY,     0, 5, 5,            fevent_ctrl_oxy_freq_notify},
  ...
  {_EVENT_CTRL_OXY_SAMPLING,        1, 5, 500,          fevent_ctrl_oxy_sampling},
  
  {_EVENT_CTRL_OXY_SW_MAN_AUTO,     0, 5, 500,          fevent_ctrl_oxy_sw_man_auto},
};
...
Struct_TimeSafety               sTimeSafety={0};
/*=======================Function Hanlde=================*/
static uint8_t fevent_ctrl_oxy_entry(uint8_t event)
{
...
uint32_t Gettick_Power_Oxy = 0;
uint32_t Gettick_Connect_Oxy = 0;
static uint8_t fevent_ctrl_oxy_read_power(uint8_t event)
{
    ...
    {
        Gettick_Power_Oxy = HAL_GetTick();
    }
    fevent_active(sEventAppCtrlOxy, event);
    return 1;
}
...
static uint8_t fevent_ctrl_oxy_pw_detect(uint8_t event)
{
    ...
    if(sStateCtrlOxy.StatePower != StatePowerBefor)
    {
        ...
        else
        {
            aData[0] = 0x00;
            aData[1] = 0x00;
            Log_EventWarnig(OBIS_WARNING_POWER, 0x01, aData);
    ...
    if(sStateCtrlOxy.StatePower == _POWER_OFF)
    {
        if(HAL_GetTick() - gettick_pw_detect >= TIME_RESEND_WARNING)
        {
            gettick_pw_detect = HAL_GetTick();
            aData[0] = 0x00;
            aData[1] = 0x00;
...
static uint8_t fevent_ctrl_oxy_state_slave(uint8_t event)
{
    ...
    if(HAL_GetTick() - Gettick_Power_Oxy > 30000 || sStateCtrlOxy.StateSensorOxy == _OXY_CONNECT)
    {
    ...
    //            UTIL_Log_Str(DBLEVEL_M, "u_app_oxy: Slave_CONNECT!\r\n");
            ...
            else
            {
                ...
                aData[0] = 0x00;
                aData[1] = 0x00;
                Log_EventWarnig(OBIS_WARNING_CONNECT_SENSOR, 0x01, aData);
    //            UTIL_Log_Str(DBLEVEL_M, "u_app_oxy: Slave_DISCONNECT!\r\n");
        ...
        {
            if(HAL_GetTick() - gettick_state_slave >= TIME_RESEND_WARNING)
            {
                gettick_state_slave = HAL_GetTick();
                aData[0] = 0x00;
                aData[1] = 0x00;
    ...
    if(sStateCtrlOxy.StateSensorOxy == _OXY_DISCONNECT)      
    {
        Gettick_Connect_Oxy = HAL_GetTick();
    }
...
static uint8_t fevent_ctrl_oxy_warning(uint8_t event)
{
    ...
    if(HAL_GetTick() - Gettick_Connect_Oxy > 15000) 
    {
        if(sParamMeasure.Oxy_Mg_L < sParamCtrlOxy.Oxy_Lower_Warning)
        {
            ...
            if(sStateCtrlOxy.StateOxyLower != StateOxyBefore)
            {
                ...
                aData[0] = 0x00;
                aData[1] = 0x00;
                Log_EventWarnig(OBIS_WARNING_OXY_LOWER, 0x01, aData);
        ...
        {
            if(HAL_GetTick() - gettick_warning_period >= TIME_RESEND_WARNING)
            {
                gettick_warning_period = HAL_GetTick();
                aData[0] = 0x00;
                aData[1] = 0x00;
    ...
    else
    {
        ...
        StateOxyBefore = _PH_BALANCE;
        sStateCtrlOxy.StateWarningOxy_Measure = _OXY_STT_OK;
    }
    
    fevent_active(sEventAppCtrlOxy, _EVENT_PUMP_WARNING);
    return 1;
}
...
static uint8_t fevent_pump_warning(uint8_t event)
{
    ...
    if(HAL_GetTick() - Gettick_Power_Oxy > 30000)
    {
        if(HAL_GetTick() - gettick_pump > TIME_RESEND_WARNING_PUMP*60000 || gettick_pump == 0)
        {
            if(sStateCtrlOxy.StateWarningOxy_Pump == _PUMP_FAIL)
            {
                for(uint8_t i = 1; i <= Number_Motor_Slave; i++)
                {
                    if( sStateSlaveMotor[i].Error_Amperage == 1)
                    {
                        aData[0] = i;
                        aData[1] = 0x00;
                        Log_EventWarnig(OBIS_WARNING_PUMP, 0x02, aData);
                    }
                    else if (sStateSlaveMotor[i].Error_Pressure == 1)
                    {
                        aData[0] = i;
                        aData[1] = 0x01;
                        Log_EventWarnig(OBIS_WARNING_PUMP, 0x02, aData);
                    }
                    else if (sStateSlaveMotor[i].Count_DisConnect >= NUMBER_CHECK_DISCONNECT_SLAVE)
                    {
                        aData[0] = i;
                        aData[1] = 0x02;
                        Log_EventWarnig(OBIS_WARNING_PUMP, 0x02, aData);
    ...
    fevent_active(sEventAppCtrlOxy, _EVENT_CTRL_OXY_LED_WARNING);
    return 1;
}

static uint8_t fevent_ctrl_oxy_led_warning(uint8_t event)
{
    if(sStateCtrlOxy.StateWarningOxy_Connect == _OXY_STT_FAIL ||
       sStateCtrlOxy.StateWarningOxy_Measure == _OXY_STT_FAIL ||
       sStateCtrlOxy.StatePower == _POWER_OFF)
    {
...
static uint8_t fevent_ctrl_oxy_reset_dcu(uint8_t event)
{
    ...
    if(sDate_Reset.Year != 0)
    {
        if(Flag_Reset_DCU == 1)
        {
            if(sTime_Reset.Hours >= 3 && sTime_Reset.Hours < 6)
            {
                if(sParamMeasure.Oxy_Mg_L > sParamCtrlOxy.Oxy_Lower_Warning)
                {
//                    Save_Array(ADDR_FLAG_RESET_DCU, (uint8_t*)0x01, 1);
        ...
        {
            if(HAL_GetTick() > 1380*60000) //Sau 23 gio Onl lien tuc chuyen sang doi reset dcu
            {
...
static uint8_t fevent_ctrl_oxy_sampling(uint8_t event)
{
    uint8_t Count_Oxy_Run = 0;
    uint8_t Level_Run = 0;

    if(sRTC.year >= 24)
    {
        if(sRTC.hour >= 18 && sRTC.hour <= 20)
        {
            Level_Run = 0;
        }
        else if(sRTC.hour >= 21 && sRTC.hour <= 23)
        {
            Level_Run = 1;
        }
        else if(sRTC.hour <= 2)
        ...
        else if(sRTC.hour >= 3 && sRTC.hour <= 5)
        {
            Level_Run = 3;
        }
        else
    ...
    if(Level_Run < 4)
    {
        ...
        if(sStateCtrlOxy.StateSafety == _OXY_SAFETY_SAMPLING)
        {
            if(Count_Oxy_Run > 1)
            {
                sGettickMs.Oxy_Safety_Sampling = sGettickMs.GettickMs;
                sParamCtrlOxy.Oxy_Measure_Control = sParamMeasure.Oxy_Mg_L;
            }
            else
            {
                if(sGettickMs.GettickMs - sGettickMs.Oxy_Safety_Sampling > (sTimeSafety.TimeSampling + 5*Level_Run)*60000)
                {
                    sStateCtrlOxy.StateSafety = _OXY_SAFETY_RUN;
                    sGettickMs.Oxy_Safety_Run = sGettickMs.GettickMs;
                    
                    sParamCtrlOxy.Oxy_Measure_Control = 0;
                }
                else
                {
                    sParamCtrlOxy.Oxy_Measure_Control = sParamMeasure.Oxy_Mg_L;
        ...
        {
            if(sGettickMs.GettickMs - sGettickMs.Oxy_Safety_Run > (sTimeSafety.TimeRun)*60000)
            {
                sStateCtrlOxy.StateSafety = _OXY_SAFETY_SAMPLING;
                sGettickMs.Oxy_Safety_Sampling = sGettickMs.GettickMs;
    ...
    fevent_active(sEventAppCtrlOxy, _EVENT_CTRL_OXY_SW_MAN_AUTO);
    return 1;
}
...
static uint8_t fevent_ctrl_oxy_sw_man_auto(uint8_t event)
{
    ...
    fevent_enable(sEventAppCtrlOxy, _EVENT_CTRL_OXY_SAMPLING);
    return 1;
}
...
//    }
    ...
    if(sStateCtrlOxy.StateDCU == 1)
    {
        if(sParamCtrlOxy.Oxy_Measure_Control < sParamCtrlOxy.Oxy_Lower)
        {
        ...
        else if(sParamCtrlOxy.Oxy_Measure_Control > sParamCtrlOxy.Oxy_Upper)
        {
...
void Save_GetTickMs(void)
{
    ...
    aData[length++] = sGettickMs.Oxy_Safety_Run >> 24;
    aData[length++] = sGettickMs.Oxy_Safety_Run >> 16;
    aData[length++] = sGettickMs.Oxy_Safety_Run >> 8;
    aData[length++] = sGettickMs.Oxy_Safety_Run;
    
    aData[length++] = sGettickMs.Oxy_Safety_Sampling >> 24;
    aData[length++] = sGettickMs.Oxy_Safety_Sampling >> 16;
    aData[length++] = sGettickMs.Oxy_Safety_Sampling >> 8;
    aData[length++] = sGettickMs.Oxy_Safety_Sampling;
    
    aData[length++] = sStateCtrlOxy.StateSafety;
...
void Init_GetTickMs(void)
{
    if(*(__IO uint8_t*)(ADDR_OXY_GETTICK) == BYTE_TEMP_FIRST)
    {
        ...
        sGettickMs.Oxy_Safety_Run = *(__IO uint8_t*)(ADDR_OXY_GETTICK+14) <<24;
        sGettickMs.Oxy_Safety_Run |= *(__IO uint8_t*)(ADDR_OXY_GETTICK+15) <<16;
        sGettickMs.Oxy_Safety_Run |= *(__IO uint8_t*)(ADDR_OXY_GETTICK+16) <<8;
        sGettickMs.Oxy_Safety_Run |= *(__IO uint8_t*)(ADDR_OXY_GETTICK+17) ;
        
        sGettickMs.Oxy_Safety_Sampling = *(__IO uint8_t*)(ADDR_OXY_GETTICK+18) <<24;
        sGettickMs.Oxy_Safety_Sampling |= *(__IO uint8_t*)(ADDR_OXY_GETTICK+19) <<16;
        sGettickMs.Oxy_Safety_Sampling |= *(__IO uint8_t*)(ADDR_OXY_GETTICK+20) <<8;
        sGettickMs.Oxy_Safety_Sampling |= *(__IO uint8_t*)(ADDR_OXY_GETTICK+21) ;
        
        sStateCtrlOxy.StateSafety = *(__IO uint8_t*)(ADDR_OXY_GETTICK+22) ;
    ...
    else
    {
        ...
        sGettickMs.Oxy_Safety_Run = 0;
        sGettickMs.Oxy_Safety_Sampling = 0;
        sStateCtrlOxy.StateSafety = _OXY_SAFETY_SAMPLING;
    }
}
...
/*-----------------------Save and Init Time Safety-----------------------*/
void        Save_TimeSafety(uint16_t TimeRun_Min, uint16_t TimeSampling_Min)
{
    uint8_t aData[8] = {0};
    uint8_t length = 0;
    
    sTimeSafety.TimeRun = TimeRun_Min;
    sTimeSafety.TimeSampling = TimeSampling_Min;
    
    aData[length++] = sTimeSafety.TimeRun >> 8;
    aData[length++] = sTimeSafety.TimeRun;
    
    aData[length++] = sTimeSafety.TimeSampling >> 8;
    aData[length++] = sTimeSafety.TimeSampling;

    Save_Array(ADDR_TIME_SAFETY, aData, length);
    Stamp_Menu_Exit();
}

void        Init_TimeSafety(void)
{
    if(*(__IO uint8_t*)(ADDR_TIME_SAFETY) == BYTE_TEMP_FIRST)
    {
        sTimeSafety.TimeRun  = *(__IO uint8_t*)(ADDR_TIME_SAFETY+2) <<8;
        sTimeSafety.TimeRun |= *(__IO uint8_t*)(ADDR_TIME_SAFETY+3);
        
        sTimeSafety.TimeSampling  = *(__IO uint8_t*)(ADDR_TIME_SAFETY+4) <<8;
        sTimeSafety.TimeSampling |= *(__IO uint8_t*)(ADDR_TIME_SAFETY+5);
        
    }
    else
    {
        sTimeSafety.TimeRun = TIME_RUN_OXY_SAFETY;
        sTimeSafety.TimeSampling = TIME_SAMPLING_OXY_SAFETY;
    }
    Stamp_Menu_Exit();
}
