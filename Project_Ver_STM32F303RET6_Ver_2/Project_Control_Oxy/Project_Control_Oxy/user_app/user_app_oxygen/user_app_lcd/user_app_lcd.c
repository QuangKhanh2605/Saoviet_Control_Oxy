...
LCD_Char_Display                    oLCD_C_Setting_Main[]=
{
  ...
  {_LCD_C_SETTING_MAIN_CYCLE,          0,3,8,           (char*)"[1] Slave Control "},
...
{
  {_LCD_C_SETTING_CYCLE_TITLE,          0,2,25,          (char*)"SETTING SLAVE "},
  {_LCD_C_SETIING_CYCLE_NUMBER_MOTOR,   0,3,8,           (char*)"[1] Motor:       "},
  {_LCD_C_SETTING_CYCLE_RUN,            0,4,8,           (char*)"[2] Cycle:      s"},
  {_LCD_C_SETTING_CYCLE_FREE,           0,5,8,           (char*)"[3] Free :      s"},
  {_LCD_C_SETTING_CYCLE_SLAVE,          0,6,8,           (char*)"[4] Send Slave   "},
...
{
  {_LCD_U_SETIING_CYCLE_NUMBER_MOTOR,   0,3,70,         &sMenuStamp.NumberMotorSlave},
  {_LCD_U_SETTING_CYCLE_RUN,            0,4,70,         &sMenuStamp.sTimeSlave.RunCtrl},
  {_LCD_U_SETTING_CYCLE_FREE,           0,5,70,         &sMenuStamp.sTimeSlave.FreeCtrl},
};
...
void DLCD_Setting_Cycle_Alter(void)
{
    ...
    static uint16_t stampNumberMotor = 0;
    ...
    if(*oLCD_U_Setting_Cycle[_LCD_U_SETIING_CYCLE_NUMBER_MOTOR].Object != stampNumberMotor)
    {
        stampNumberMotor = *oLCD_U_Setting_Cycle[_LCD_U_SETIING_CYCLE_NUMBER_MOTOR].Object;
        oLCD_U_Setting_Cycle[_LCD_U_SETIING_CYCLE_NUMBER_MOTOR].Status = 1;
    }
...
void DLCD_Setting_Cycle_Entry(void)
{
    ...
    oLCD_C_Setting_Cycle[_LCD_C_SETIING_CYCLE_NUMBER_MOTOR].Status = 1;
...
void        DLCD_Setting_Cycle(void)
{
    ...
    for(uint8_t i = 0; i < _LCD_U_SETTING_CYCLE_END; i++)
    {
        ...
        {
          if(i == _LCD_U_SETIING_CYCLE_NUMBER_MOTOR)
          {
           GLCD_Write_Float_Not_Zero(*oLCD_U_Setting_Cycle[i].Object,0,  oLCD_U_Setting_Cycle[i].Rol,oLCD_U_Setting_Cycle[i].Col);
