...
static uint8_t fevent_check_man_auto(uint8_t event)
{
    ...
    static uint8_t StateControlBefore = _RS485_MOTOR_ON;
    ...
    if(StateBefore != sParamMotor.StateMachine)
    {
        ...
        {
            case _MACHINE_AUTO:
                if(sInforRs485.State_OnOff_Motor == _RS485_MOTOR_OFF)
                {
                    OFF_Motor_3_Phase();
                ...
                StateControlBefore = sInforRs485.State_OnOff_Motor;
                break;
...
//                fevent_disable(sEventAppMotor, _EVENT_SAMPLING_AMPERAGE);
    ...
    if(sParamMotor.StateMachine == _MACHINE_AUTO)
    {
        if(StateControlBefore != sInforRs485.State_OnOff_Motor)
        {
            switch(sInforRs485.State_OnOff_Motor)
            {
                case _RS485_MOTOR_ON:
                case _RS485_MOTOR_FAIL:
                    if(StateControlBefore == _RS485_MOTOR_OFF || sParamMotor.StateMotor == _MOTOR_OFF)
                    {
                        fevent_active(sEventAppMotor, _EVENT_ON_MOTOR);
                        fevent_disable(sEventAppMotor, _EVENT_SAMPLING_AMPERAGE);
                        fevent_disable(sEventAppMotor, _EVENT_MOTOR_CHECK_AMPERAGE);
...
static uint8_t fevent_motor_check_amperage(uint8_t event)
{
    ...
    if(sMeasureMotor.AmperagePhase_1 <=5 && sMeasureMotor.AmperagePhase_2 <=5 &&sMeasureMotor.AmperagePhase_3 <=5)
    {
...
//    {
//        sParamMotor.WaitSamplingPressureGetTick_Ms = HAL_GetTick();
//    }
    MeasurePressure = sMeasureMotor.Pressure;
    ...
    if(HAL_GetTick() - sParamMotor.WaitSamplingPressureGetTick_Ms > 60000 || sParamMotor.Amperage_Option == _AMPERAGE_OPTION_OFF)
    {
...
//    static uint8_t StateSensorMotorBefore = _MOTOR_OK;
    ...
    if(sParamMotor.StatusAmperage == _MOTOR_FAIL )
      ...
      StatusMotor = _MOTOR_FAIL;
    else if(sInforRs485.State_OnOff_Motor == _RS485_MOTOR_FAIL && sParamMotor.StateMachine == _MACHINE_AUTO)
      StatusMotor = _MOTOR_FAIL;
    else 
...
void        Save_OptionWarning(uint16_t Option_Amperage, uint16_t Option_Pressure, uint16_t Constant_Pressure)
{
    uint8_t aData[10] = {0};
    uint8_t length = 0;
    ...
    sParamMotor.Pressure_Constant = Constant_Pressure;
    ...
    aData[length++] = sParamMotor.Pressure_Constant >> 8;
    aData[length++] = sParamMotor.Pressure_Constant;
...
void        Init_OptionWarning(void)
{
    if(*(__IO uint8_t*)(ADDR_OPTION_SETTING) == BYTE_TEMP_FIRST)
    {
        ...
        sParamMotor.Pressure_Constant  = *(__IO uint8_t*)(ADDR_OPTION_SETTING+6) <<8;
        sParamMotor.Pressure_Constant |= *(__IO uint8_t*)(ADDR_OPTION_SETTING+7);
    }
    else
    {
        sParamMotor.Amperage_Option = _AMPERAGE_OPTION_ON;
        sParamMotor.Pressure_Option = _PRESSURE_OPTION_ON;
        sParamMotor.Pressure_Constant = DEFAULT_PRESSURE_CONSTANT;
    }
    Stamp_Menu_Exit();
}
...
uint16_t Measure_Pressure_P1;
uint16_t Measure_Pressure_P2;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if(htim->Instance == htim2.Instance) 
    {
      ...
      if(sParamADC.Number_Sampling == NUMBER_SAMPLING_AVG_ADC)
      {
        ...
        Measure_Pressure_P1 = Algorithm_Amperage_to_Pressure(sParamADC.Amperage_Pressure_Ap1, 4000, 0, 20000, sParamMotor.Pressure_Constant*10);
        Measure_Pressure_P2 = Algorithm_Amperage_to_Pressure(sParamADC.Amperage_Pressure_Ap2, 4000, 0, 20000, sParamMotor.Pressure_Constant*10);
        
        if(Measure_Pressure_P1 >= Measure_Pressure_P2)
        {
            sMeasureMotor.Pressure = Measure_Pressure_P1;
            sMeasureMotor.Pressure_Chanel = _CHANEL_P1;
        }
        else
        {
            sMeasureMotor.Pressure = Measure_Pressure_P2;
            sMeasureMotor.Pressure_Chanel = _CHANEL_P2;
        }
