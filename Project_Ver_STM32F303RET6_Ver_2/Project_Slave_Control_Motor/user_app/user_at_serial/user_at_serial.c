...
//			id ENUM				  kind process			     str Received
        ...
        {_GET_PASSWORD_OXY,     _fGET_PASSWORD_OXY,          {(uint8_t*)"at+passwordpump?",16}},         //Ex: at+password?
        {_SET_PASSWORD_OXY,     _fSET_PASSWORD_OXY,          {(uint8_t*)"at+passwordpump=",16}},         //Ex: at+password=000000
        
        {_GET_OPTION_AMPERAGE,  _fGET_OPTION_AMPERAGE,       {(uint8_t*)"at+optionamperage?",18}},   //Ex: at+optionamperage?
        {_SET_OPTION_AMPERAGE,  _fSET_OPTION_AMPERAGE,       {(uint8_t*)"at+optionamperage=",18}},   //Ex: at+optionamperage=1 (ON option Amperage)
        
        {_GET_OPTION_PRESSURE,  _fGET_OPTION_PRESSURE,       {(uint8_t*)"at+optionpressure?",18}},   //Ex: at+optionpressure?
        {_SET_OPTION_PRESSURE,  _fSET_OPTION_PRESSURE,       {(uint8_t*)"at+optionpressure=",18}},   //Ex: at+optionpressure=1 (Use option Pressure chanel P1 (max chanel P4))
        
        {_GET_CONSTANT_PRESSURE,_fGET_CONSTANT_PRESSURE,     {(uint8_t*)"at+constantpressure?",20}},
        {_SET_CONSTANT_PRESSURE,_fSET_CONSTANT_PRESSURE,     {(uint8_t*)"at+constantpressure=",20}},
        
        {_GET_PRESSURE_UPPER,   _fGET_PRESSURE_UPPER,        {(uint8_t*)"at+pressureupper?",17}},    //Ex: at+pressureupper?
        {_SET_PRESSURE_UPPER,   _fSET_PRESSURE_UPPER,        {(uint8_t*)"at+pressureupper=",17}},    //Ex: at+pressureuppre=30 (set pressures upper 3.0 bar) 
        
        {_GET_PRESSURE_LOWER,   _fGET_PRESSURE_LOWER,        {(uint8_t*)"at+pressurelower?",17}},    //Ex: at+pressurelower?
        {_SET_PRESSURE_LOWER,   _fSET_PRESSURE_LOWER,        {(uint8_t*)"at+pressurelower=",17}},    //Ex: at+pressurelower=5  (set pressure lower 0.5 bar)
         
        {_GET_AMPERAGE_UPPER,   _fGET_AMPERAGE_UPPER,        {(uint8_t*)"at+amperageupper?",17}},    //Ex: at+amperageupper?
        {_SET_AMPERAGE_UPPER,   _fSET_AMPERAGE_UPPER,        {(uint8_t*)"at+amperageupper=",17}},    //Ex: at+amperageupper=
        
        {_GET_AMPERAGE_DELTA,   _fGET_AMPERAGE_DELTA,        {(uint8_t*)"at+amperagedelta?",17}},    //Ex: at+amperagedelta?
        {_SET_AMPERAGE_DELTA,   _fSET_AMPERAGE_DELTA,        {(uint8_t*)"at+amperagedelta=",17}},    //Ex: at+amperagedelta=30 (set amperage delta 30%)
        
        {_CALIB_AMPERAGE,       _fCALIB_AMPERAGE,            {(uint8_t*)"at+calibamperage=",17}},
        
        {_GET_ID_SLAVE,         _fGET_ID_SLAVE,              {(uint8_t*)"at+idslave?",11}},
        {_SET_ID_SLAVE,         _fSET_ID_SLAVE,              {(uint8_t*)"at+idslave=",11}},
...
void        _fGET_PASSWORD_OXY (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
...
void        _fSET_PASSWORD_OXY (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
...
void        _fGET_OPTION_AMPERAGE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    if(sParamMotor.Amperage_Option == _AMPERAGE_OPTION_ON)
        DCU_Respond(PortConfig, (uint8_t*)"AMPERAGE_OPTION_ON", 18, 0);
    else
        DCU_Respond(PortConfig, (uint8_t*)"AMPERAGE_OPTION_OFF", 19, 0);
#endif
}

void        _fSET_OPTION_AMPERAGE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        if(TempU32 <= _AMPERAGE_OPTION_ON)
        {
            Save_OptionWarning(TempU32, sParamMotor.Pressure_Option, sParamMotor.Pressure_Constant);
            DCU_Respond(PortConfig, (uint8_t*)"OK", 2, 0);
        }
        else
            DCU_Respond(PortConfig, (uint8_t*)"ERROR", 5, 0);
...
void        _fGET_OPTION_PRESSURE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    switch(sParamMotor.Pressure_Option)
    {
        case _PRESSURE_OPTION_OFF:
          DCU_Respond(PortConfig, (uint8_t*)"PRESSURE_OPTION_OFF", 19, 0);
          break;
          
        case _PRESSURE_OPTION_ON:
          DCU_Respond(PortConfig, (uint8_t*)"PRESSURE_OPTION_ON", 18, 0);
          break;
...
void        _fSET_OPTION_PRESSURE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        TempU32 = Convert_String_To_Dec(str_Receiv->Data_a8 , length);
        if(TempU32 <= _PRESSURE_OPTION_ON)
        {
            Save_OptionWarning(sParamMotor.Amperage_Option, TempU32, sParamMotor.Pressure_Constant);
            DCU_Respond(PortConfig, (uint8_t*)"OK", 2, 0);
        }
        else
        {
...
void        _fGET_CONSTANT_PRESSURE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint8_t aTemp[25] = "Pressure_Const:";   //11 ki tu dau tien
    uint16_t length = 15;

    Convert_Point_Int_To_String_Scale (aTemp, &length, (int)(sParamMotor.Pressure_Constant), 0x00);

	DCU_Respond(PortConfig, aTemp, length, 0);
#endif
}

void        _fSET_CONSTANT_PRESSURE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        TempU32 = Convert_String_To_Dec(str_Receiv->Data_a8 , length);
        if(TempU32 >= PRESSURE_CONSTANT_MIN && TempU32 <= PRESSURE_CONSTANT_MAX)
        {
            Save_OptionWarning(sParamMotor.Amperage_Option, sParamMotor.Pressure_Option, TempU32);
...
void        _fGET_PRESSURE_UPPER (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint8_t aTemp[25] = "Pressure_Upper:";   //11 ki tu dau tien
    uint16_t length = 15;

    Convert_Point_Int_To_String_Scale (aTemp, &length, (int)(sParamMotor.Pressure_Upper), 0xFF);

	DCU_Respond(PortConfig, aTemp, length, 0);
#endif
}

void        _fSET_PRESSURE_UPPER (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        TempU32 = Convert_String_To_Dec(str_Receiv->Data_a8 , length);
        if(TempU32 > sParamMotor.Pressure_Lower && TempU32 <= PRESSURE_UPPER_MAX)
        {
            Save_PressureWarning(TempU32, sParamMotor.Pressure_Lower);
            DCU_Respond (PortConfig, (uint8_t *)"OK", 2, 1);
        }
        else
            DCU_Respond(PortConfig, (uint8_t *)"FAIL", 4, 0);
...
void        _fGET_PRESSURE_LOWER (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint8_t aTemp[25] = "Pressure_Lower:";   //11 ki tu dau tien
    uint16_t length = 15;

    Convert_Point_Int_To_String_Scale (aTemp, &length, (int)(sParamMotor.Pressure_Lower), 0xFF);

	DCU_Respond(PortConfig, aTemp, length, 0);
#endif
}

void        _fSET_PRESSURE_LOWER (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        TempU32 = Convert_String_To_Dec(str_Receiv->Data_a8 , length);
        if(TempU32 < sParamMotor.Pressure_Upper )
        {
            Save_PressureWarning(sParamMotor.Pressure_Upper, TempU32);
            DCU_Respond (PortConfig, (uint8_t *)"OK", 2, 1);
        }
        else
            DCU_Respond(PortConfig, (uint8_t *)"FAIL", 4, 0);
...
void        _fGET_AMPERAGE_UPPER (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint8_t aTemp[25] = "Amperage_Upper:";   //11 ki tu dau tien
    uint16_t length = 15;

    Convert_Point_Int_To_String_Scale (aTemp, &length, (int)(sParamMotor.Amperage_Upper), 0xFF);

	DCU_Respond(PortConfig, aTemp, length, 0);
#endif
}

void        _fSET_AMPERAGE_UPPER (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        TempU32 = Convert_String_To_Dec(str_Receiv->Data_a8 , length);
        if(TempU32 <= AMPERAGE_UPPER_MAX )
        {
            Save_AmperageWarning(TempU32, sParamMotor.Amperage_Delta);
            DCU_Respond (PortConfig, (uint8_t *)"OK", 2, 1);
        }
        else
            DCU_Respond(PortConfig, (uint8_t *)"FAIL", 4, 0);
...
void        _fGET_AMPERAGE_DELTA (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint8_t aTemp[25] = "Amperage_Delta:";   //11 ki tu dau tien
    uint16_t length = 15;

    Convert_Point_Int_To_String_Scale (aTemp, &length, (int)(sParamMotor.Amperage_Delta), 0x00);

	DCU_Respond(PortConfig, aTemp, length, 0);
#endif
}

void        _fSET_AMPERAGE_DELTA (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        TempU32 = Convert_String_To_Dec(str_Receiv->Data_a8 , length);
        if(TempU32 <= AMPERAGE_DELTA_MAX )
        {
            Save_AmperageWarning(sParamMotor.Amperage_Upper, TempU32);
            DCU_Respond (PortConfig, (uint8_t *)"OK", 2, 1);
        }
        else
            DCU_Respond(PortConfig, (uint8_t *)"FAIL", 4, 0);
...
void        _fCALIB_AMPERAGE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        if(TempU32 == 0 )
        {
            Save_AmperageCalib(sMeasureMotor.AmperagePhase_1_ADC, TempU32, sParamMotor.Amperage_Calib_Slope_Measure, sParamMotor.Amperage_Calib_Slope_Real);
            DCU_Respond (PortConfig, (uint8_t *)"OK", 2, 1);
        }
        else if(TempU32 <= AMPERAGE_UPPER_MAX)
        {
            Save_AmperageCalib(sParamMotor.Amperage_Calib_Point_0_Measure, sParamMotor.Amperage_Calib_Point_0_Real, sMeasureMotor.AmperagePhase_1_ADC, TempU32);
            DCU_Respond (PortConfig, (uint8_t *)"OK", 2, 1);
        }
        else
        {
            Save_AmperageCalib(AMPERAGE_CALIB_POINT_0_MEASURE_DEFAULT, AMPERAGE_CALIB_POINT_0_REAL_DEFAULT, \
                               AMPERAGE_CALIB_SLOPE_MEASURE_DEFAULT, AMPERAGE_CALIB_SLOPE_REAL_DEFAULT);
            DCU_Respond(PortConfig, (uint8_t *)"FAIL", 4, 0);
        }
    }
    else
...
void        _fGET_ID_SLAVE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint8_t aTemp[25] = "ID_Slave:";   //11 ki tu dau tien
    uint16_t length = 9;

    Convert_Point_Int_To_String_Scale (aTemp, &length, (int)(sInforRs485.ID_Slave ), 0x00);

	DCU_Respond(PortConfig, aTemp, length, 0);
#endif
}

void        _fSET_ID_SLAVE (sData *str_Receiv, uint16_t Pos)
{
#ifdef USING_APP_SLAVE_MOTOR
    uint32_t TempU32 = 0;
    if( str_Receiv->Data_a8[0] >= '0' && str_Receiv->Data_a8[0] <= '9')
    {
        ...
        TempU32 = Convert_String_To_Dec(str_Receiv->Data_a8 , length);
        if(TempU32 <= SLAVE_ID_MAX && TempU32 > 0)
        {
