...
#else
    HAL_UARTEx_ReceiveToIdle_DMA(&huart1, aUART_RX_DMA_1, sizeof(aUART_RX_DMA_1));
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if (huart->Instance == uart_debug.Instance)
	{
        *(sUartDebug.Data_a8 + sUartDebug.Length_u16++) = UartDebugRxByte;
        ...
        fevent_enable(sEventAppComm, _EVENT_UART_RX_DEBUG);
	}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
    ...
    if (huart->Instance == uart_debug.Instance)
    {
        UART_Get_Data_Debug(&sUartDebug, sizeof(UartDebugBuff), 
    ...
    if (huart->Instance == uart_sim.Instance)
    {
        UART_Get_Data_Debug(&sUartSim, sizeof(uartSimBuffReceive), 
    ...
    if (huart->Instance == huart1.Instance)
    {
        UART_Get_Data_Debug(&sUart485, sizeof(aUART_485_DATA), 
