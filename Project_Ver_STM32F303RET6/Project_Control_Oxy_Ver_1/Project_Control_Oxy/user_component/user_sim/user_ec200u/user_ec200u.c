...
uint8_t aSimStepBlockInit[14] = 
{
	...
	_SIM_AT_BAUD_RATE,          //3
    _SIM_AT_UPDATE_TIMEZONE,    //4
	_SIM_AT_CHECK_RSSI,         //5
	_SIM_AT_GET_IMEI,           //6
    _SIM_AT_DTR_MODE,           //7
    
    _SIM_CALLING_LINE,          //8
    _SIM_SMS_FORMAT_TEXT,       //9    
    _SIM_SMS_NEW_MESS_IND,      //10
//    _SIM_GPS_CFG_AUTO_RUN,      //11
//    _SIM_GPS_TURN_ON,           //12
    
    _SIM_NET_CEREG,             //13
    _SIM_AT_GET_ID,             //14
    _SIM_AT_CHECK_SIM,          //15
};
...
const sCommand_Sim_Struct aSimEC200UStep[] =
{
    ...
    {   _SIM_AT_UPDATE_TIMEZONE,    at_callback_success,        at_callback_failure,        "OK",                   "AT+CTZU=0\r"       },  // AT+CTZU=0 +GMT=0, AT+CTZU=3 +GMT=28 (28/4 = 7)
   ...
   //SMS
	...
	{   _SIM_MQTT_PUB_FB_2, 	    _Cb_MQTT_PUBLISH_2,         at_callback_failure,        "SEND OK",          NULL       	        },  // 4,"40020001"
    //Ping
...
//    uint8_t Pos = uart_string->Length_u16 + 5;
//    
//    while(Pos > 0)
//    {
...
//    }
//    
//    if(uart_string->Data_a8[Pos+1] == '0' && uart_string->Data_a8[Pos+2] == '0')
//    {
//        sData  strCheck = {(uint8_t*) aSimEC200UStep[_SIM_NET_GET_RTC].at_response, strlen(aSimEC200UStep[_SIM_NET_GET_RTC].at_response)};
//        //Call Func Get sTime BTS
//        Sim_Common_Get_Stime_BTS (&strCheck, uart_string);
...
//    else
//    {
////        Sim_Common_Send_AT_Cmd(&uart_sim, (uint8_t*)"AT+CCLK=\"22/22/22,22:22:22+00\"\r", sizeof("AT+CCLK=\"01/01/01,00:00:00+00\"\r")-1, 1000);
////        HAL_Delay(3000);
//        Sim_Common_Send_AT_Cmd(&uart_sim, (uint8_t*)"AT+CTZU=0\r", sizeof"AT+CTZU=0\r"-1, 1000);
//        HAL_Delay(3000);
//        __disable_irq();
//        //Reset
