...
sEvent_struct               sEventAppCtrlOxy[]=
{
  ...
  {_EVENT_CTRL_OXY_LOG_EEPROM,      1, 5, 5000,         fevent_ctrl_oxy_log_eeprom},
...
static uint8_t fevent_ctrl_oxy_state_slave(uint8_t event)
{
    ...
    if(sStateCtrlOxy.StateSensorOxy != StateSlaveBefore)
    {
    ...
    if(sRTC.year > 20)
    {
        if(handle_save_log == 0)
        {
            if(sRTC.min%10 == 0)
            {
                ...
                aWrite[1] = sRTC.hour;
                aWrite[2] = sRTC.min;
                aWrite[3] = sParamMeasure.Oxy_Mg_L >> 8;
                aWrite[4] = sParamMeasure.Oxy_Mg_L ;
               
                CAT24Mxx_Write_Array(sOxyLogEEPROM.Addr_Oxy_Log_Start + ((sRTC.hour*60 + sRTC.min)/10)*5, aWrite, 5);
            }
        }
        else
        {
...
static uint8_t fevent_send_log_eeprom(uint8_t event)
{
    ...
    while(1)
    {
        CAT24Mxx_Read_Array(sOxyLogEEPROM.Addr_Oxy_Log_Start + countLog*5, aRead, 5);
        if(aRead[0] > 0 && aRead[0] <= 31)
        {
            ...
            Insert_String_To_String(aSendLog, &length, (uint8_t*)",",0 , 1);
            Convert_Point_Int_To_String_Scale (aSendLog, &length, (int)(Oxy_Log), 0xFE);
        }
        countLog++;
        if(countLog%20==0)
        {
...
uint8_t AppCtrlOxy_Packet_TSVH (uint8_t *pData)
{
    ...
    if(sStateCtrlOxy.StateDCU == 1)
    {
        ...
        //-------- State Connect Sensor Oxy----
        AppCtrlOxy_Packet_Data(pData, &length, OBIS_ENVI_STATE_SENSOR_OXY, &sStateCtrlOxy.StateSensorOxy, 1, 0xAA);
    
        //-------- State Connect Sensor pH----
        AppCtrlOxy_Packet_Data(pData, &length, OBIS_ENVI_STATE_SENSOR_PH, &sStateCtrlOxy.StateSensorPH, 1, 0xAA);
...
//    uint8_t *TempU8;
    ...
    switch (Obis)
    {
        ...
        case OBIS_ENVI_STATE_SENSOR_OXY:
        case OBIS_ENVI_STATE_SENSOR_PH:
