...
{
  {_EVENT_RS485_ENTRY,              0, 5, 20000,            fevent_rs485_entry},            //Doi slave khoi dong moi truyen opera
...
/*--------------------Hoi du lieu tu Slave-------------------*/
    ...
    switch(sKindMode485.Trans)
    {
        ...
        case _RS485_RESET_SENSOR:
            aData[0] = 0x00;
            aData[1] = 0x01;
            ModRTU_Master_Write_Frame(&sFrame, sIdSlave485.Oxy, 0x10, 0x0300 , 1, aData);
            break;
        ...
        case _RS485_RESET_CALIB:
            switch(CycleReset)
            {
                case _RS485_RESET_0:
                    aData[0] = 0x00;
                    aData[1] = 0x10;    //0x10 Reset Calib 0%
                    ModRTU_Master_Write_Frame(&sFrame, sIdSlave485.Oxy, 0x10, 0x0220 , 1, aData);
                    break;
    ...
    //Check Kind Send
    if(sKindMode485.Trans == _RS485_RESET_SENSOR)
    {
        fevent_enable(sEventAppRs485, _EVENT_RS485_ENTRY);
        fevent_disable(sEventAppRs485, _EVENT_RS485_RECEIVE_HANDLE);
        fevent_disable(sEventAppRs485, _EVENT_RS485_RECEIVE_COMPLETE);
        return 1;
    }
    else if(sKindMode485.Trans == _RS485_OPERA_OXY)
...
//                    Average_MeasureOxy(Oxy_Mg_L, Oxy_Percent);
                ...
                if(Reset_Sensor == 0)
                {
                    ...
                    Reset_Sensor = 1;
                    sKindMode485.Trans = _RS485_RESET_SENSOR;
                }
                break;
