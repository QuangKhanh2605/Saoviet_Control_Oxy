...
static uint8_t fevent_ctrl_oxy_log_eeprom(uint8_t event);
static uint8_t fevent_send_log_eeprom(uint8_t event);
/*========================Struct========================*/
sEvent_struct               sEventAppCtrlOxy[]=
{
  {_EVENT_CTRL_OXY_ENTRY,           1, 5, 25000,        fevent_ctrl_oxy_entry}, //doi sensor do on dinh thi bat dau xet cac truong hop
...
//  {_EVENT_CTRL_OXY_SW_NOTIFY,       1, 5, 500,          fevent_ctrl_oxy_sw_notify},
  ...
  {_EVENT_CTRL_OXY_PW_DETECT,       1, 0, 1000,         fevent_ctrl_oxy_pw_detect},
  
  {_EVENT_CTRL_OXY_STATE_SLAVE,     0, 5, 2000,         fevent_ctrl_oxy_state_slave},
  
  {_EVENT_CTRL_OXY_WARNING,         0, 5, 2000,         fevent_ctrl_oxy_warning},
...
//  {_EVENT_CTRL_OXY_FREQ_NOTIFY,     0, 5, 5,            fevent_ctrl_oxy_freq_notify},
  ...
  {_EVENT_CTRL_OXY_LOG_EEPROM,      1, 5, 15000,        fevent_ctrl_oxy_log_eeprom},
  {_EVENT_SEND_LOG_EEPROM,          0, 5, 3000,         fevent_send_log_eeprom},
};
...
static uint8_t fevent_ctrl_oxy_pw_detect(uint8_t event)
{
    ...
    if(sStateCtrlOxy.StatePower != sStateCtrlOxy.StatePower_Before)
    {
        gettick_pw_detect = HAL_GetTick();
        Save_StateWarning();
        if(sStateCtrlOxy.StatePower == _POWER_ON)  
        {
            Log_EventWarnig(OBIS_WARNING_POWER, 0x01);
...
//            Reset_Chip();
        ...
        sStateCtrlOxy.StatePower_Before = sStateCtrlOxy.StatePower;
    }
...
static uint8_t fevent_ctrl_oxy_state_slave(uint8_t event)
{
    ...
    if(sStateCtrlOxy.StateSensorOxy != sStateCtrlOxy.StateSensorOxy_Before)
    {
        gettick_state_slave = HAL_GetTick();
        Save_StateWarning();
        if(sStateCtrlOxy.StateSensorOxy == _OXY_CONNECT)      
        {
            Log_EventWarnig(OBIS_WARNING_CONNECT_SENSOR, 0x01);
        ...
        sStateCtrlOxy.StateSensorOxy_Before = sStateCtrlOxy.StateSensorOxy;
    }
...
static uint8_t fevent_ctrl_oxy_warning(uint8_t event)
{
    ...
    {
      if(sGettickMs.GettickMs - sGettickMs.Oxy_Warning > sParamCtrlOxy.TimeWarning * 60000)
      {
        sStateCtrlOxy.StateOxyLower = _OXY_LOW;
        OnOff_Warning(ON_RELAY);
        if(sStateCtrlOxy.StateOxyLower != sStateCtrlOxy.StateOxyLower_Before)
        {
            gettick_warning_period = HAL_GetTick();
            Log_EventWarnig(OBIS_WARNING_OXY_LOWER, 0x00);
            Save_StateWarning();
            sStateCtrlOxy.StateOxyLower_Before = sStateCtrlOxy.StateOxyLower;
        }
      }
    }
    else
    {
        ...
        sGettickMs.Oxy_Warning = sGettickMs.GettickMs;
        if(sStateCtrlOxy.StateOxyLower != sStateCtrlOxy.StateOxyLower_Before)
        {
...
static uint8_t fevent_ctrl_oxy_log_eeprom(uint8_t event)
{
    static uint8_t handle_save_log = 0;
    uint16_t addrWrite = 0;
    ...
    if(sRTC.year > 20)
    {
        if(handle_save_log == 0)
        {
            if(sRTC.min%10 == 0)
            {
                handle_save_log = 1;

                aWrite[length++] = sRTC.date;
                aWrite[length++] = sRTC.hour;
                aWrite[length++] = sRTC.min;
                aWrite[length++] = sParamMeasure.Oxy_Mg_L >> 8;
                aWrite[length++] = sParamMeasure.Oxy_Mg_L;
               
                if(Calculator_Addr_Read_Write_EEPROM((sRTC.hour*60 + sRTC.min)/10 + 1, &addrWrite)==1)
                    CAT24Mxx_Write_Array(addrWrite, aWrite, length);
            }
        }
        else
        {
            if(sRTC.min%5 == 0 && sRTC.min%10 !=0)
            {
                handle_save_log = 0;
            }
        }
...
static uint8_t fevent_send_log_eeprom(uint8_t event)
{
    static uint8_t countLog = 0;
    uint16_t Oxy_Log = 0;
    uint8_t aSendLog[300]={0};
    uint16_t length = 0;
    uint8_t aRead[5]={0};
    uint16_t AddrRead = 0;
    ...
    while(1)
    {
        if(Calculator_Addr_Read_Write_EEPROM(countLog + 1, &AddrRead) == 1)
            CAT24Mxx_Read_Array(AddrRead, aRead, 5);
        
        if(aRead[0] > 0 && aRead[0] <= 31)
        {
            Oxy_Log = aRead[3]<<8 | aRead[4];
            
            Insert_String_To_String(aSendLog, &length, (uint8_t*)"|",0 , 1);
            Convert_Point_Int_To_String_Scale (aSendLog, &length, (int)(aRead[0]), 0x00);
            Insert_String_To_String(aSendLog, &length, (uint8_t*)"/",0 , 1);
            Convert_Point_Int_To_String_Scale (aSendLog, &length, (int)(aRead[1]), 0x00);
            Insert_String_To_String(aSendLog, &length, (uint8_t*)":",0 , 1);
            Convert_Point_Int_To_String_Scale (aSendLog, &length, (int)(aRead[2]), 0x00);
            Insert_String_To_String(aSendLog, &length, (uint8_t*)",",0 , 1);
            Convert_Point_Int_To_String_Scale (aSendLog, &length, (int)(Oxy_Log), 0x00);
        }
        countLog++;
        ...
        {
            Insert_String_To_String(aSendLog, &length, (uint8_t*)"|",0 , 1);
            DCU_Respond(sOxyLogEEPROM.KindSendLog, aSendLog, length, 0);
            countLog = 0;
            return 1;
        }
        if(countLog%21==0)
...
//uint32_t Write_EEPROM = 1;
//uint32_t Read_EEPROM  = 0;
//static uint8_t fevent_test_eeprom(uint8_t event)
//{
//    static uint8_t kind_send = 0;
//
//        if(kind_send == 0)
//        {
//            write_e[0] = Write_EEPROM >> 24;
//            write_e[1] = Write_EEPROM >> 16;
//            write_e[2] = Write_EEPROM >> 8;
//            write_e[3] = Write_EEPROM ;
...
//            Read_EEPROM = write_e[0] << 24;
//            Read_EEPROM |= write_e[1] << 16;
//            Read_EEPROM |= write_e[2] << 8;
//            Read_EEPROM |= write_e[3] ;
...
void Control_Oxy(uint8_t KindOxy, uint8_t State)
{
    ...
    switch(KindOxy)
    {
        case _OXY_1:
          ...
          ControlRelay(RELAY_6, StateRelay, _RL_DEBUG);
          break;
          
        case _OXY_2:
          ControlRelay(RELAY_2, StateRelay, _RL_DEBUG);
          ControlRelay(RELAY_7, StateRelay, _RL_DEBUG);
          break;
          
        case _OXY_3:
          ControlRelay(RELAY_3, StateRelay, _RL_DEBUG);
          ControlRelay(RELAY_8, StateRelay, _RL_DEBUG);
          break;
          
        case _OXY_4:
          ControlRelay(RELAY_4, StateRelay, _RL_DEBUG);
          ControlRelay(RELAY_9, StateRelay, _RL_DEBUG);
          break;
...
void        Init_TimeCtrlOxy(void)
{
    ...
    else
    {
        ...
        sParamCtrlOxy.TimeWarning = TIMEWARNING_DEFAULT;
    }
    Stamp_Menu_Exit();
}
...
void Init_TimeSlave(void)
{
    if(*(__IO uint8_t*)(ADDR_CTRL_OXY_RUN_FREE) == BYTE_TEMP_FIRST)
    {
        ...
        if(sTimeSlave.RunCtrl > TIME_MAX)
...
/*========================Save and Init State Warning=====================*/
void Save_StateWarning(void)
{
    uint8_t aData[8] = {0};
    uint8_t length = 0;
    
    aData[length++] = sStateCtrlOxy.StatePower;
    aData[length++] = sStateCtrlOxy.StateSensorOxy;
    aData[length++] = sStateCtrlOxy.StateOxyLower;

    Save_Array(ADDR_STATE_WARNING, aData, length);  
}
void Init_StateWarning(void)
{
    if(*(__IO uint8_t*)(ADDR_STATE_WARNING) == BYTE_TEMP_FIRST)
    {
        sStateCtrlOxy.StatePower_Before     = *(__IO uint8_t*)(ADDR_STATE_WARNING+2);
        sStateCtrlOxy.StateSensorOxy_Before = *(__IO uint8_t*)(ADDR_STATE_WARNING+3);
        sStateCtrlOxy.StateOxyLower_Before  = *(__IO uint8_t*)(ADDR_STATE_WARNING+4);
    }
}
