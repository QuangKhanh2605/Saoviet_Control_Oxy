...
{   
    ...
    switch ( AppIVT_Set_Mode_Led() )
    {
        case _LED_MODE_ONLINE_INIT:
            sEventAppIVT[event].e_period = 1000;
            LED_Toggle (_LED_STATUS);
            break;
        case _LED_MODE_CONNECT_SERVER:
            if (sEventAppIVT[event].e_period != 430)
            {
                sEventAppIVT[event].e_period = 430;
                LED_Off (_LED_STATUS);
            } else
            {
                sEventAppIVT[event].e_period = 70;
                LED_On (_LED_STATUS);
            }
            break;
        case _LED_MODE_UPDATE_FW:
            sEventAppIVT[event].e_period = 100;
            LED_Toggle (_LED_STATUS);
            break;      
...
uint8_t AppIVT_Set_Mode_Led (void)
{
    uint8_t Result = 0;
