...
//			id ENUM				  kind process			     str Received
        ...
        {_GET_BAUDRATE_SLAVE,   _fGET_BAUDRATE_SLAVE,       {(uint8_t*)"at+baudrate?",12}},
        {_SET_BAUDRATE_SLAVE,   _fSET_BAUDRATE_SLAVE,       {(uint8_t*)"at+baudrate=",12}},
        
        {_SET_TIME_RUN,         _fSET_TIME_RUN,             {(uint8_t*)"at+run=",7}},
        {_SET_TIME_FREE,        _fSET_TIME_FREE,            {(uint8_t*)"at+free=",8}},
...
//    sParamPcBox.StatePcBox = _STT_PCBOX_RESET;
...
//#ifdef USING_APP_PC_BOX
//    fevent_active(sEventAppPcBox, _EVENT_OFF_PC_BOX);
//    sParamPcBox.StatePcBox = _STT_PCBOX_DISCONNECT;
//#endif
//}
//
///*
...
//    uint16_t lengthRespond = 0;
//    
//    Insert_String_To_String(aDataRespond, &lengthRespond, (uint8_t*)"CyclePcBox: ",0 , 13);
//    
...
//          break;
//          
//        case _STT_PCBOX_RESET:
//          Insert_String_To_String(aDataRespond, &lengthRespond, (uint8_t*)"PcBox RESET",0 , 11);
//          break;
//          
//        default:
//          break;
...
{
    uint8_t aTemp[60] = "Baudrate: ";   //11 ki tu dau tien
    uint16_t length = 10;

    Convert_Point_Int_To_String_Scale (aTemp, &length, (int)(sParamSlave.Baudrate), 0x00);
...
void        _fSET_BAUDRATE_SLAVE (sData *strRecei, uint16_t Pos)
{
    ...
    if( strRecei->Data_a8[0] >= '0' && strRecei->Data_a8[0] <= '9')
    {
        ...
        TempU32 = Convert_String_To_Dec(strRecei->Data_a8 , length);
        
        if(Save_BaudrateSlave(TempU32) == 1)
            
            HAL_UART_Transmit(&uart_debug, (uint8_t*)"OK\r\n", 4, 1000);
        else
            HAL_UART_Transmit(&uart_debug, (uint8_t*)"ERROR\r\n", 7, 1000);
    }
}
...
void        _fSET_TIME_RUN (sData *strRecei, uint16_t Pos)
{
    ...
    if( strRecei->Data_a8[0] >= '0' && strRecei->Data_a8[0] <= '9')
    {
        ...
        if(TempU32 <= 999)
        {
            Save_TimeCycleSlave(TempU32, sParamSlave.TimeFree);
            if(sParamMenu.StateMenu == _MENU_SETUP_RUN)
                Dispaly_Number_Led7seg(sParamSlave.TimeRun, 2);
            else if(sParamMenu.StateMenu == _MENU_SETUP_FREE)
...
void        _fSET_TIME_FREE (sData *strRecei, uint16_t Pos)
{
    ...
    if( strRecei->Data_a8[0] >= '0' && strRecei->Data_a8[0] <= '9')
    {
        ...
        if(TempU32 <= 999)
        {
            Save_TimeCycleSlave(sParamSlave.TimeRun , TempU32);
            if(sParamMenu.StateMenu == _MENU_SETUP_RUN)
                Dispaly_Number_Led7seg(sParamSlave.TimeRun, 2);
            else if(sParamMenu.StateMenu == _MENU_SETUP_FREE)
                Dispaly_Number_Led7seg(sParamSlave.TimeFree, 2);
